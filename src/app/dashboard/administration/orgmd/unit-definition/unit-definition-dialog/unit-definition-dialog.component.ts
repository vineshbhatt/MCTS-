...
@Component({
  selector: 'app-unit-definition-dialog',
  templateUrl: './unit-definition-dialog.component.html',
  styleUrls: ['./unit-definition-dialog.component.scss']
})
export class UnitDefinitionDialogComponent implements OnInit {
  NodeData: FormGroup;
  header: string;
  actionType: string;
  runSpinner = false;
  ...
  ngOnInit() {
    ...
    if (this.data.action === 'update') {
      this.actionType = 'edit_unit';
      this.fillForm();
    } else if (this.data.action === 'insert') {
      this.actionType = 'add_unit';
    }
  }

  fillForm() {
    this.NodeData.get('OUTID').setValue(this.data.element.OUTID);
  ...
  save() {
    ...
    if (this.NodeData.valid) {
      ...
      const obj = {
        templateName: 'OU_TYPE_VER',
        objectID: this.NodeData.get('OUTID').value,
